...
4 source files, no nested modules:

- **`main.rs`** — Entry point. CLI parsing (clap), stdin-to-TTY redirect via `libc::dup/dup2`, Tokio runtime setup, main TUI event loop (`run_app`), and `--json-output` test mode (`run_json_mode`) that drives the app through stdin commands (`CMD:C-c`, `CMD:Enter`, etc.) and serializes state as NDJSON.
- **`app.rs`** — Core state and logic. `App` struct holds all state (lines, selection, scroll, filter, view mode). Handles keyboard events with vim bindings, renders TUI via ratatui (breadcrumb + list/detail view + status bar). `AppState` is the serializable snapshot used by e2e tests.
...
- **JSON test mode**: `--json-output` flag replaces the TUI with a line-by-line state serializer. E2e tests feed input + `CMD:` prefixed keystrokes via stdin and parse the JSON output. This avoids needing a real terminal in tests.
...
- E2e tests in `tests/e2e.rs` use `--json-output` mode: they pipe lines and `CMD:KeySequence` commands to logq's stdin, then assert on the serialized `AppState` snapshots.
